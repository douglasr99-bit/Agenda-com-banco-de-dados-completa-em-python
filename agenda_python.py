...
def valida_faixa_inteiro(pergunta, inicio, fim, padrao=None):
    while True:
        try:
            ...
            print(f'Valor deve ser um inteiro entre {inicio} e {fim}.')
...
def valida_faixa_inteiro_ou_branco(pergunta, inicio, fim):
    while True:
        try:
            ...
            print(f'Valor deve ser um inteiro entre {inicio} e {fim} ou em branco para cancelar.')
...
class Nome:
    ...
    def __str__(self):
        return self.nome
    def __repr__(self):
        return "<Classe {3} em 0x{0:x} com Nome: '{1}' Chave: {2}>".format(id(self), self.nome, self.__chave, type(self).__name__)
...
class DBAgenda:
    ...
    def pesquisaNome(self, nome):
        if not isinstance(nome, DBNome):
            raise TypeError('Nome deve ser do tipo DBNome.')
        achado = self.conexao.execute('select * from nomes where nome = ?', (nome.nome,)).fetchone()
        ...
        try:
            ...
            for telefone in registro.telefones:
                if telefone.id is None:
                    ...
                    cur.execute('update telefones set numero = ?, id_tipo = ? , id_nome = ? where id = ?', (telefone.numero, telefone.tipo.id, registro.nome.id, telefone.id))
...
class Menu:
    ...
    def adcionaopcao(self, nome, funcao):
...
class AppAgenda:
    ...
    def mostra_dados(dados):
        ...
        for i, telefone in enumerate(dados.telefones):
            print("Telefone {0}: {1}".format(i+1, telefone))
    @staticmethod
    ...
    def __init__(self, banco):
        ...
        self.menu = Menu()
        self.menu.adcionaopcao("Novo", self.novo)
        self.menu.adcionaopcao("Editar", self.alterar)
        self.menu.adcionaopcao("Apagar", self.apaga)
        self.menu.adcionaopcao("Listar", self.lista)
        self.ultimo_nome = None
    def pede_tipo_telefone(self, padrao = None):
        for i, tipo in enumerate(self.agenda.tiposTelefone):
            print(f"[{i+1}] {tipo}", end = None)
        t = valida_faixa_inteiro("Tipo de telefone: ",1, len(self.agenda.tiposTelefone), padrao)
        return self.agenda.tiposTelefone[t - 1]
    ...
    def novo(self):
        ...
        novo_tipo_telefone = self.pede_tipo_telefone()
        if nulo_ou_vazio(novo):
            print("Nome não pode ser vazio.")
            return
        nome = DBNome(novo)
        telefone = DBTelefone(novo_telefone, novo_tipo_telefone)
        if self.pesquisa(nome) != None:
            print("Nome já existe.")
            return
        registro = DBDadoAgenda(nome)
        registro.telefones.adiciona(telefone)
        self.mostra_dados_telefone(registro)
    ...
    def alterar(self):
        ...
        if p != None:
            ...
            novo_nome = AppAgenda.pede_nome()
            if not nulo_ou_vazio(novo_nome):
                p.nome.nome = novo_nome
    ...
    def menu_telefones(self, dados):
        while True:
            ...
            if operacao == 'A' and len(dados.telefones) > 0:
                self.alterar_telefones(dados)
            elif operacao == 'D' and len(dados.telefones) > 0:
                self.apagar_telefone(dados)
            elif operacao == 'N':
    ...
    def apagar_telefone(self, dados):
        t = valida_faixa_inteiro_ou_branco("Número do telefone a apagar: ",0, len(dados.telefones)-1)
        if t == None:
            return
        dados.telefones.remove(dados.telefones[t])
    def alterar_telefones(self, dados):
        t = valida_faixa_inteiro_ou_branco("Número do telefone a alterar: ",0, len(dados.telefones)-1)
        if t == None:
            return
        telefone = dados.telefones[t]
        print("\nTelefone: %s" % telefone)
        print("\nDigite enter caso não queira alterar o número")
        novo_numero = AppAgenda.pede_telefone()
        if not nulo_ou_vazio(novo_numero):
            telefone.numero = novo_numero
        print("\nDigite enter caso não queira alterar o tipo")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Erro: informe o nome do arquivo do banco de dados como argumento.")
        print("      agenda.py <banco_de_dados>")
